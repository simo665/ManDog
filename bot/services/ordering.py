"""
The code has been updated to update rating view creation to track disabled users.
"""
import logging
import discord
...
class OrderingService:
    ...
    async def complete_order(self, order_id: str):
        ...
        try:
            ...
            # NEW: Remove buyer from queue if they were queued for this item
            await self.bot.db_manager.remove_from_queue(buyer.id, order_data['listing_id'])
    ...
    async def handle_rating_submission(self, order_id: str, rater_id: int, rated_id: int, rating: int, comment: str = ""):
        ...
        try:
            ...
            if rating_config:
                threshold = rating_config[0]['low_rating_threshold']

            # Check if rating needs admin approval
            if rating < threshold and rating_config and rating_config[0]['admin_channel_id']:
                await self.create_admin_moderation_ticket(order_id, rater_id, rated_id, rating, comment)
                return True

            # Good rating or no admin config - process immediately
            await self.process_rating(rater_id, rated_id, rating, comment, guild_id, order_id)
    ...
    async def create_admin_moderation_ticket(self, order_id: str, rater_id: int, rated_id: int, rating: int, comment: str):
        ...
        try:
            ...
            # Get configured admin channel
            rating_config = await self.bot.db_manager.execute_query(
                "SELECT admin_channel_id FROM guild_rating_configs WHERE guild_id = $1",
                guild.id
            )

            if not rating_config or not rating_config[0]['admin_channel_id']:
                logger.warning(f"No admin channel configured for guild {guild.id}")
                return

            admin_channel = guild.get_channel(rating_config[0]['admin_channel_id'])
            if not admin_channel:
                logger.warning(f"Admin channel {rating_config[0]['admin_channel_id']} not found")
                return

            # Store pending rating in database
            await self.bot.db_manager.execute_command(
                """
                INSERT INTO ratings (rater_id, rated_id, guild_id, order_id, rating, comment, status, created_at)
                VALUES ($1, $2, $3, $4, $5, $6, 'pending', $7)
                """,
                rater_id, rated_id, guild.id, order_id, rating, comment, datetime.now(timezone.utc)
            )

            rater = guild.get_member(rater_id)
            rated = guild.get_member(rated_id)

            # Send moderation embed to configured admin channel
            ...
            moderation_embed.add_field(name="👤 Rater", value=rater.mention if rater else f"User {rater_id}", inline=True)
            moderation_embed.add_field(name="👥 Rated User", value=rated.mention if rated else f"User {rated_id}", inline=True)
            ...
            logger.info(f"Sent rating moderation request to {admin_channel.name} for order {order_id}")
        ...
        try:
            if approved:
                ...
                await self.bot.db_manager.execute_command(
                    """
                    UPDATE ratings 
                    SET status = 'approved', admin_id = $1, reviewed_at = $2
                    WHERE order_id = $3 AND rater_id = $4 AND rated_id = $5 AND status = 'pending'
                    """,
                    admin_id, datetime.now(timezone.utc), order_id, rater_id, rated_id
                )
                ...
                if rating_data and len(rating_data['ratings']) == 2:
                    self.pending_ratings.pop(order_id, None)
            else:
                ...
                await self.bot.db_manager.execute_command(
                    """
                    UPDATE ratings 
                    SET status = 'rejected', admin_id = $1, reviewed_at = $2
                    WHERE order_id = $3 AND rater_id = $4 AND rated_id = $5 AND status = 'pending'
                    """,
                    admin_id, datetime.now(timezone.utc), order_id, rater_id, rated_id
                )
    ...
    async def process_rating(self, rater_id: int, rated_id: int, rating: int, comment: str, guild_id: int, order_id: str = None):
        ...
        try:
            ...
            await self.bot.db_manager.execute_command(
                ...
                """,
                rater_id, rated_id, guild_id, order_id, rating, comment, datetime.now(timezone.utc)
            )
    ...
    async def update_user_reputation(self, user_id: int):
        ...
        try:
            # Calculate new reputation from approved ratings only
            reputation_data = await self.bot.db_manager.execute_query(
                ...
                FROM ratings 
                WHERE rated_id = $1 AND status = 'approved'
                """,
                user_id
            )
            ...
            if reputation_data:
                ...
                await self.bot.db_manager.execute_command(
                    ...
                    DO UPDATE SET 
                        ...
                        updated_at = $4
                    """,
                    user_id, avg_rating, total_ratings, datetime.now(timezone.utc)
                )
