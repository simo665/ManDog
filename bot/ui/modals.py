...
The code has been reviewed and no changes are needed based on the instructions.
"""Modal for creating a listing with time/date selection and queue support for "All Items" WTS listings."""
...
class ItemQueueModal(discord.ui.Modal):
    """Modal for entering the specific item to queue for when listing "All Items"."""

    def __init__(self, bot, listing_data: Dict[str, Any]):
        super().__init__(title="Specify Item for Queue")
        self.bot = bot
        self.listing_data = listing_data

        self.item_name_input = discord.ui.TextInput(
            label="Specific Item Name",
            placeholder="Enter the exact item you want to queue for",
            max_length=200,
            required=True,
            style=discord.TextStyle.short
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        """Handle item name submission for queue."""
        try:
            specific_item_name = self.item_name_input.value.strip()

            # Add specific item name to listing data
            self.listing_data['specific_item'] = specific_item_name
            ...
            embed = discord.Embed(
                title="📅 Schedule Your Listing",
                description=f"When would you like to be available for **{self.listing_data['specific_item']}**?",
                color=0x3B82F6,
                timestamp=datetime.now(timezone.utc)
            )
            ...
            embed.add_field(
                name="🎯 Item",
                value=self.listing_data['specific_item'],
                inline=True
            )

            embed.add_field(
                name="📦 Quantity",
                value=str(self.listing_data['quantity']),
                inline=True
            )
            ...
            logger.error(f"Error in item queue modal submission: {e}")
            await interaction.response.send_message(
                "❌ An error occurred while processing your item queue",
                ephemeral=True
            )
